...
import { LOGIN_NAME, PAGE_NOT_FOUND_NAME, REDIRECT_NAME, REGISTER_NAME } from './constant';
...
export function createRouterGuards(router: Router, whiteNameList: WhiteNameList) {
  router.beforeEach(async (to, from, next) => {
    ...
    if (userStore.token) {
      ...
      // not login
      if (
        whiteNameList.some((n) => n === to.name) ||
        to.name === REGISTER_NAME ||
        to.name === LOGIN_NAME
      ) {
        // 在免登录名单或去往登录/注册页面，直接进入
        next();
      } else {
